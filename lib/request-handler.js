...
exports.saveLink = function(req, res) {
  ...
  Link.findOne({url: uri}, function(err, link) {
    if (link) {
      console.log('This is the FOUND link retrieved from the db: ', link);
      ...
      util.getUrlTitle(uri, function(err, title) {
        ...
        var newLink = new Link({url: uri, title: title, baseUrl: req.headers.origin});
    ...
    if (user) {
      user.comparePassword(password, function(isMatch) {
        if (isMatch) {
          ...
          console.log('BAD ACCOUNT/PASSWORD YE HACKER');
          res.redirect('/login');
        }
      })
    } else {
